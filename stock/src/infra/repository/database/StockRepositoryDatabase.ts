...
export default class StockRepositoryDatabase implements StockRepository {
    ...
    async getStockEntries (idItem: number): Promise<StockEntry[]> {
        const stockEntriesData = await this.connection.query("select * from cccat8.stock_entry where id_item = $1", [idItem]);
        const stockEntries = [];
    ...
    async save (stockEntry: StockEntry): Promise<void> {
        await this.connection.query("insert into cccat8.stock_entry (id_item, operation, quantity) values ($1, $2, $3 )",
        [stockEntry.idItem, stockEntry.operation, stockEntry.quantity]);
    }

    async clear (): Promise<void> {
        await this.connection.query("delete from cccat8.stock_entry", []); 
    }
}